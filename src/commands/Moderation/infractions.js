...
class Infractions extends Command {
  ...
  async run(message, args) {
    const user =
      message.mentions[0] ||
      message.channel.guild.members.get(args[0]) ||
      message.channel.guild.members.find((x) => x.username === args[0]);
    ...
    if (!user) {
      ...
      if (warns.length < 1) {
        ...
        warns.forEach((s) => {
          ...
          embed.fields.push({
            name: this.firstLUC(s.typeOf) + " | #" + s.case,
            value: `<@!${s.moderator}> - ${s.reason}`,
            inline: false,
          });
