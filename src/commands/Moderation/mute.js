...
class Mute extends Command {
  ...
  async run(message, args) {
    ...
    if (!args[0])
      return error(`**${authorTag}** Specify a valid member to mute!`);
    ...
    if (memberToMute[0].id == message.author.id)
      return error(`**${authorTag}** You can mute yourself!`);
    ...
    const highestTargetRole = Math.max.apply(
      Math,
      message.channel.guild.members.get(memberToMute[0].id).roles.map((x) => {
        return x.position;
      })
    );
    ...
    if (
      ...
      return error(`**${authorTag}** You can't mute that member!`);
    ...
    if (
      ...
      return error(`**${targetTag}** Is already muted!`);
    ...
    message.channel.send({
      embed: {
        description: `<:yes:762884751832252417> **${targetTag}** has been muted by **${authorTag}**\nReason: ${
