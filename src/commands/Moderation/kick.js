...
class Kick extends Command {
  ...
  async run(message, args) {
    ...
    if (
      !userMentions[0] ||
      !message.channel.guild.members.get(userMentions[0].id)
    ) {
      return message.channel.send({
        embed: {
          description: `<:no:762884741069275156> **${authorTag}** Specify a valid member to kick!`,
          color: 0xff4949,
        },
      });
    }
    ...
    if (userMentions[0].id == message.author.id) {
      return message.channel.send({
        embed: {
          description: `<:no:762884741069275156> **${authorTag}** You can't kick yourself.`,
          color: 0xff4949,
        },
      });
    }
    ...
    const highestTargetRole = Math.max.apply(
      Math,
      memberToBan.roles.map((x) => {
        return x.position;
      })
    );
    if (
      highestAuthorRole <= highestTargetRole &&
      message.author.id != message.channel.guild.ownerID
    ) {
      return message.channel.send({
        embed: {
          description: `<:no:762884741069275156> **${authorTag}** You can't kick that member!`,
          color: 0xff4949,
        },
      });
    }
    ...
    await memberToBan
      ...
      .then(() => {
        return message.channel.send({
          embed: {
            description: `<:yes:762884751832252417> **${targetTag}** Has been kicked by ${authorTag}\nReason: ${
      ...
      .catch((e) => {
        return message.channel.send({
          embed: {
            description: `<:no:762884741069275156> Something went wrong... I couldn't kick **${targetTag}**`,
            color: 0xff4949,
          },
        });
      });
