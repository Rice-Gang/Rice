...
class Unmute extends Command {
    ...
    async run(message, args) {
        ...
        if (!args[0]) return error(`**${authorTag}** Specify a valid member to unmute!`);

        const memberToMute = message.mentions || this.rice.users.get(args[0]) || this.rice.users.find(u => u.username == args[0]);

        if (!memberToMute[0] || !message.channel.guild.members.get(memberToMute[0].id)) return error(`**${authorTag}** Specify a valid member to unmute!`);

        if (memberToMute[0].id == message.author.id) return error(`**${authorTag}** You are not muted`);
        ...
        if (!message.channel.guild.members.get(memberToMute[0].id).roles.find(r => r == muteRole.id)) return error(`**${targetTag}** Isn't muted!`);

        await message.channel.guild.members.get(memberToMute[0].id).removeRole(muteRole.id);

        message.channel.send({ embed: { description: `<:yes:762884751832252417> **${targetTag}** has been unmuted by **${authorTag}**`, color: 0x3cb474 } });
