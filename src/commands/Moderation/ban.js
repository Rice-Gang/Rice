...
class Ban extends Command {
    ...
    async run(message, args) {
        ...
        const userMentions = message.mentions[0] || this.rice.users.get(args[0]) || this.rice.users.find(x => x.username == args[0]);
        ...
        const highestTargetRole = Math.max.apply(Math, memberToBan.roles.map((e) => {
            return e.position;
        }));

        if (parseInt(highestAuthorRole) <= parseInt(highestTargetRole) && message.author.id != message.channel.guild.ownerID) {
            return message.channel.send({ embed: { description: `<:no:762884741069275156> **${authorTag}** You can"t ban that member!`, color: 0xff4949 } });
        }
        //<:yes:762884751832252417>
        if (memberToBan.user.id == message.channel.guild.ownerID) {
            return message.channel.send({ embed: { description: `<:no:762884741069275156> **${authorTag}** You can"t ban the server owner!`, color: 0xff4949 } });
        }
        const reason = args.slice(1).join(" ");
        const targetTag = `${memberToBan.user.username}#${memberToBan.user.discriminator}`;
        await memberToBan.ban().then(() => {
            return message.channel.send({ embed: { description: `<:yes:762884751832252417> **${targetTag}** Has been banned by ${authorTag}\nReason: ${reason || "No Reason Provided"}`, color: 0x3cb474 } });
        }).catch((e) => {
            return message.channel.send({ embed: { description: `<:no:762884741069275156> Something went wrong... I couldn't ban **${targetTag}**`, color: 0xff4949 } });
        })
