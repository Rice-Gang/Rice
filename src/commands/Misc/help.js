...
class Help extends Command {
  ...
  async run(message, args, developers) {
    if (args[0]) {
      const cmd =
        this.rice.commands.get(args[0]) ||
        this.rice.commands.get(this.rice.aliases.get(args[0]));
      ...
      if (cmd.config.botPerms)
        embed.fields.push({
          ...
          value:
            cmd.config.botPerms.map((x) => `\`${x}\``).join(", ") || "None",
          inline,
        });
      if (cmd.config.memberPerms)
        embed.fields.push({
          ...
          value:
            cmd.config.memberPerms.map((x) => `\`${x}\``).join(", ") || "None",
          inline,
        });
      ...
      categories.sort().forEach((ct) => {
        const cmds = this.rice.commands.filter(
          (cmd) => cmd.help.category === ct
        );
        embed.fields.push({
          name: emojis[ct.toLowerCase()] + " " + ct,
          value: cmds.map((cmd) => `\`${cmd.help.name}\``).join(", "),
        });
      });
