...
class Ustats extends Command {
  ...
  async run(msg, args) {
    ...
    let user =
      ...
      this.rice.users.find((x) => x.id == args[0]) ||
      this.rice.users.find(
        (x) =>
          x.username.toLowerCase() + "#" + x.discriminator ==
          args.join(" ").toLowerCase()
      ) ||
      msg.author;
    let join_time =
      Date.now() -
      msg.channel.guild.members.find((x) => x.id == user.id).joinedAt;
    ...
    embed.fields.push({
      name: "Joined",
      value: `${Math.floor(join_time / 86400000)} Days ${Math.floor(
        (join_time / (1000 * 60 * 60)) % 24
      )} Hours ${Math.floor(
        (join_time / (1000 * 60)) % 60
      )} Minutes ${Math.floor((join_time / 1000) % 60)} Seconds`,
      inline: true,
    });
    embed.fields.push({
      name: "Created",
      value: `${Math.floor(create_time / 86400000)} Days ${Math.floor(
        (create_time / (1000 * 60 * 60)) % 24
      )} Hours ${Math.floor(
        (create_time / (1000 * 60)) % 60
      )} Minutes ${Math.floor((create_time / 1000) % 60)} Seconds`,
      inline: true,
    });
    ...
    if (game) {
      try {
        ...
        embed.fields.push({
          name: `Status create`,
          value: `${Math.floor(game_create / 86400000)} Days ${Math.floor(
            (game_create / (1000 * 60 * 60)) % 24
          )} Hours ${Math.floor(
            (game_create / (1000 * 60)) % 60
          )} Minutes ${Math.floor((game_create / 1000) % 60)} Seconds`,
          inline: false,
        });
      } catch (e) {
        PassThrough;
