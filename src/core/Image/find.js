class find{
    static async find(msg, args, rice){
        let lastimg = false;
        let user = msg.mentions[0] || msg.channel.guild.members.find(x => x.username.toLowerCase() == args.join(" ").toLowerCase()) || rice.users.find(x => x.id == args[0])
        if(user){
        ...
        if(!user){
            await msg.channel.getMessages().then(message => {
                message.forEach(data => {
                    ...
                    if(!data.attachments[0].url.includes("png")){if(!data.attachments[0].url.includes("jpeg")){if(!data.attachments[0].url.includes("jpg")){return;}}}
