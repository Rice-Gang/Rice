...
module.exports = class {
  ...
  async run(message) {
    ...
    if (prefixRegex.test(message.content))
      return message.channel.send(`My prefix in here is \`${prefix}\``);
    ...
    const command =
      this.rice.commands.get(cmd) ||
      this.rice.commands.get(this.rice.aliases.get(cmd));

    if (!command) return;

    if (
      command.help.category === "Developer" &&
    ...
    if (cooldown.has(`${message.author.id}_${command.help.name}`)) {
      return message.channel
        .sendError(`You are still in cooldown for this command\
            \nCooldown: ${command.config.cooldown / 1000} Second(s)`);
    } else {
      if (!developers.includes(message.author.id)) {
        this.rice.cooldown.add(`${message.author.id}_${command.help.name}`);
        setTimeout(() => {
          this.rice.cooldown.delete(
            `${message.author.id}_${command.help.name}`
          );
    ...
    if (message.channel.guild) {
      ...
      if (neededPerms.length > 0) {
        ...
        return message.channel.sendError(
          `Looks like I am missing some permissions for that command, here they are: ${needed}`
        );
      }
      ...
      if (neededPerms.length > 0) {
        return message.channel.sendError(
          `Looks like you are missing some permissions for that command... \n\n here they are: ${neededPerms
            .map((p) => "`" + p + "`")
            .join(", ")}`
        );
      }
